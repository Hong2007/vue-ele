  ...
  <div class="shopCart">
    <div class="content" @click="toggleList">
      ...
      <div class="content-right" @click.stop.prevent="pay">
        <div class="pay" :class="isEnough">{{payDesc}}</div>
      </div>
    </div>
    <div class="ball-container">
      <transition-group name="drop" @before-enter="beforeEnter" @enter="enter" @after-enter="afterEnter">
        <div class="ball" v-for="(ball,index) in balls" :key="index" v-show="ball.show">
          <div class="inner inner-hook icon-remove_circle_outline"></div>
        </div>
      </transition-group>
    </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      beforeEnter(ele) { // 小球下落前的运动函数钩子
        let len = this.balls.length;
        while (len--) {
          let ball = this.balls[len];
          if (ball.show) {
            let rect = ball.el.getBoundingClientRect(); // 获取小球的盒模型
            let x = rect.left - 32; // 小球x轴方向运动距离
            let y = -(window.innerHeight - rect.top - 22); // 小球y轴方向运动距离
            // console.log(x);
            // console.log(y);
            ele.style.display = ''; // 让小球显示
